...
df_polyline_aline <- decode_pl(a_line$routes$overview_polyline$points)
df_polyline_redline <- decode_pl(red_line$routes$overview_polyline$points)
df_polyline_route5 <- decode_pl(route_5$routes$overview_polyline$points)
df_polyline_route21 <- decode_pl(route_21$routes$overview_polyline$points)
df_polyline_route18 <- decode_pl(route_18$routes$overview_polyline$points)
df_polyline_route6 <- decode_pl(route_6$routes$overview_polyline$points)
...
server <- function(input, output) {
  ...
  observe({
    if (input$mtroutes) {
      leafletProxy("map") %>% 
        addTiles() %>%
        addPolylines(data = df_polyline_greenline, lat = ~lat, lng = ~lon, color = "black", layerId = "greenline") %>%
        addPolylines(data = df_polyline_blueline, lat = ~lat, lng = ~lon, color = "black", layerId = "blueline") %>% 
        addPolylines(data = df_polyline_aline, lat = ~lat, lng = ~lon, color = "black", layerId = "aline") %>% 
        addPolylines(data = df_polyline_redline, lat = ~lat, lng = ~lon, color = "black", layerId = "redline") %>% 
        addPolylines(data = df_polyline_route5, lat = ~lat, lng = ~lon, color = "black", layerId = "5line") %>% 
        addPolylines(data = df_polyline_route21, lat = ~lat, lng = ~lon, color = "black", layerId = "21line") %>% 
        addPolylines(data = df_polyline_route18, lat = ~lat, lng = ~lon, color = "black", layerId = "18line") %>% 
        addPolylines(data = df_polyline_route6, lat = ~lat, lng = ~lon, color = "black", layerId = "6line")
    } 
    else {
      ...
      removeShape(proxy, layerId = "5line")
      removeShape(proxy, layerId = "21line")
      removeShape(proxy, layerId = "18line")
      removeShape(proxy, layerId = "6line")
    }
  })
